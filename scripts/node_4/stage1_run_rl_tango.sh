...
# Check if the first argument is empty - if so, throw an error and exit
if [ -z "$1" ]; then
...
if [ -z "$2" ]; then
    echo "Error: DISCRIMINATOR_MODEL_PATH is required"
    echo "Usage: $0 GENERATOR_MODEL_PATH DISCRIMINATOR_MODEL_PATH"
    exit 1
fi

# Create a log directory with a timestamp
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_DIR=./logs/stage1_${TIMESTAMP}
mkdir -p $LOG_DIR

export VERL_DEBUG_LEVEL=DEBUG   # Set the debug level to "DEBUG"
export VERL_LOG_FILE=$LOG_DIR/stage1.log  # Configure the log file path
export PYTHONUNBUFFERED=1   # Force Python output to be unbuffered for real-time display

train_files="['./data/deepmath_103k_rl_math/train_easy.parquet']"
...
# Generate like ['./data/eval_benchmarks/MATH500/test.parquet', './data/eval_benchmarks/AIME2024/test.parquet', ...]
test_files="["
...
VERIFIER_MAX_RESPONSE_LENGTH=16384
...
VERIFIER_LABEL_EMA_DECAY=0.8    # Use EMA to reduce label noise
