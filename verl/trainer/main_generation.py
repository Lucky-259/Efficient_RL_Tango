...
def main_task(config):
    ...
    local_path = copy_to_local(config.model.path) # if config.model.path is not local, download it from the web to the local cache
    ...
    for batch_idx in range(start_batch, num_batch):
        ...
        for n_sample in range(config.data.n_samples):
            output_padded = wg.generate_sequences(data_padded) # Using Ray worker groups for parallel response generation
