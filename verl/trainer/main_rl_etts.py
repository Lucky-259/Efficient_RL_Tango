"""Main script for RL Efficient TTS training."""
import os
import ray
import hydra
from verl.trainer.main_ppo import get_custom_reward_fn
from verl.utils.reward_score.format import format_compute_score
from verl.trainer.ppo.rl_etts_trainer import RLEttsTrainer

@hydra.main(config_path='config', config_name='rl_etts_trainer', version_base=None)
def main(config):
    run_rletts(config)

def run_rletts(config) -> None:
...
class TaskRunner:
    def run(self, config):
        ...
        elif reward_manager_name == "etts":
            from verl.workers.reward_manager import EttsRewardManager
            reward_manager_cls = EttsRewardManager
        else:
            raise NotImplementedError
        ...
        etts_reward_fn = reward_manager_cls(tokenizer=gen_tokenizer, num_examine=0)

        verifier_reward_fn = None

        trainer = RLEttsTrainer(
            config=config,
